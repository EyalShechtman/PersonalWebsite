...
float layeredNoise(vec2 fragPx){
    ...
    vec2 p = mod(fragPx + vec2(uTime * 15.0, -uTime * 10.0), 512.0);
    vec2 q = rot30() * p;
    float n = 0.0;
    n += 0.5 * hash21(q);
    n += 0.3 * hash21(q * 2.0 + 17.0);
    n += 0.2 * hash21(q * 4.0 + 47.0);
    return n;
}
...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      // Throttle updates when not fully visible for better performance
