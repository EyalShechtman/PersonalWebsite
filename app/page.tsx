...
const ProjectCard = memo(({ project, idx }: { project: typeof allProjects[0], idx: number }) => (
  <Link
    ...
    className="bg-green-700/5 backdrop-blur-md rounded-2xl p-6 border border-green-300/10 hover:bg-green-700/10 transition-all hover:-translate-y-1 hover:shadow-2xl group block"
  >
    <div className="flex justify-between items-start mb-4">
      <h4 className="text-xl font-bold text-green-200 group-hover:text-green-100 transition-all">
        {project.title}
      </h4>
      <div className="p-2 hover:bg-green-700/10 rounded-lg transition-colors text-green-300/60 group-hover:text-green-200">
    ...
    {(project.stars || project.forks) && (
      <div className="flex gap-4 mb-3 text-sm text-green-300/60">
        {project.stars && <span>⭐ {project.stars} stars</span>}
        {project.forks && <span>🍴 {project.forks} forks</span>}
      </div>
    )}
    <p className="text-green-300/70 mb-4 leading-relaxed line-clamp-3">
      {project.description}
    </p>
    <div className="flex flex-wrap gap-2 mb-4">
      {project.tech.slice(0, 3).map((tech, i) => (
        <span
          key={i}
          className="px-3 py-1 bg-green-700/10 border border-green-300/20 text-green-200 rounded-full text-sm font-medium"
        >
          {tech}
        </span>
      ))}
      {project.tech.length > 3 && (
        <span className="px-3 py-1 text-green-300/50 text-sm">
          +{project.tech.length - 3} more
        </span>
      )}
    </div>
    <div className="border-t border-green-300/10 pt-4">
      <div className="flex items-center gap-2 text-sm text-green-300/60 group-hover:text-green-200/80 transition-colors">
  ...
  return (
    <div className="min-h-screen bg-green-950 relative overflow-hidden">
      ...
      {!isLowEndDevice && (
        <div className="fixed inset-0 z-0">
          <PrismaticBurst
            ...
            colors={['#00ff00', '#00cc00', '#009900', '#33ff33', '#66ff66', '#006600', '#99ff99', '#00ff33']}
            distort={10}
            hoverDampness={0.08}
            rayCount={18}
            mixBlendMode="screen"
      ...
      <div className="relative z-10 pb-24 md:pb-0">
        ...
        <section id="home" className="pt-20 md:pt-8">
          <ContainerScroll
            titleComponent={
              <div className="space-y-6 px-4">
                <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-6 text-green-300/80 text-sm md:text-base">
                ...
                <h1 className="text-5xl md:text-6xl lg:text-8xl font-black text-green-200 leading-tight tracking-tight font-[family-name:var(--font-orbitron)]">
                  ...
                  <span className="block mt-2 font-[family-name:var(--font-space-grotesk)] text-green-200">
                    Web Experiences
                  </span>
                </h1>
                <p className="text-lg md:text-xl text-green-300/80 max-w-2xl mx-auto leading-relaxed font-[family-name:var(--font-space-grotesk)] mb-8 md:mb-12">
          ...
          >
            ...
            <div className="w-full h-full overflow-y-auto px-6 py-8">
              <h3 className="text-3xl md:text-4xl font-bold mb-8 text-green-200 text-center">
                Featured Projects
              </h3>
        ...
        >
          <h3 className="text-4xl font-bold mb-12 text-green-200">
            All Projects
          </h3>
        ...
        <section className="max-w-6xl mx-auto px-6 py-20">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(skills).map(([category, items]) => (
              <div
                key={category}
                className="bg-green-700/5 rounded-2xl p-6 border border-green-300/10 hover:bg-green-700/10 transition-colors"
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {items.map((skill, i) => (
                    <span 
                      key={i}
                      className="px-3 py-1 bg-green-700/10 text-green-200/90 rounded-lg text-sm hover:bg-green-700/20 transition-colors"
                    >
                      {skill}
                    </span>
                  ))}
        ...
        <section id="resume" className="max-w-6xl mx-auto px-6 py-20">
          <h3 className="text-4xl font-bold mb-8 text-green-200 text-center">
            Get In Touch
          </h3>
          <div className="flex flex-col items-center gap-6">
            <p className="text-green-300/70 text-center max-w-2xl">
            ...
            <div className="flex gap-4">
              <Link
                ...
                target="_blank"
                className="px-8 py-4 bg-green-700/5 backdrop-blur-md border border-green-300/10 rounded-xl font-semibold hover:bg-green-700/10 transition-all text-green-200 flex items-center gap-2"
              >
        ...
        {/* Footer */}
        <footer className="border-t border-green-300/10 mt-20 bg-green-900/40">
